...
def get_user_website_rules(user_id:int,session:Session):
    return session.query(UserWebsiteRule).filter(UserWebsiteRule.user_id==user_id).all()
...
def delete_website_rule(website_rule_id:int,session:Session):
    website_rule=(session.query(WebsiteRule).filter(WebsiteRule.id==website_rule_id).first())
    
    if website_rule is None:
        raise WebsiteRuleNotFoundException()
    
    session.query(UserWebsiteRule).filter(UserWebsiteRule.website_rule_id==website_rule_id).delete(synchronize_session=False)
